...
void CTP_Native_ConfManageDlg::OnBnClickedButton1()
{
	...
	*/
	list<ConferenceStatusEx> allConfList;
	TPSDKResponseWithPageEx<list<ConferenceStatusEx>> result;
	int iCurrentPage = 1;
	bool bLoop = true;
	do 
	{
		...
		result = queryScheduleConferencesEx(queryConfig);
		if ( 0 == result.resultCode )
		{
			m_editResultPanel.SetWindowText(_T("查询已调度会议的状态成功"));
			if ( iCurrentPage >= result.pagesInfo.totalPages )
			{
				bLoop = false;
			} 
			else if ( iCurrentPage < result.pagesInfo.totalPages )
			{
				iCurrentPage++;
			}
			allConfList.splice(allConfList.end(),result.result);
		}
		else
		{
			bLoop = false;
			CString s; 
			s.Format("查询已调度会议的状态失败；返回码：%d",result.resultCode);
			m_editResultPanel.SetWindowText(s);
		}
	} while ( true == bLoop );

	int i = 0;
	for ( list<ConferenceStatusEx>::iterator iter = allConfList.begin(); iter != allConfList.end(); iter++ )
	{
		m_conferenceList.push_back(*iter);
		m_listConference.InsertItem(i,iter->id.c_str());
		m_listConference.SetItemText(i, 1, iter->name.c_str());
