...
void CTPOrgaMgr::SetURL(char *baseURL, char *baseMethod)
{
	sprintf_s(m_strURL,TP_D_URI_LEN,"%s/%s",baseURL,baseMethod);
}

TPSDKResponseEx<list<OrganizationItemEx>>  CTPOrgaMgr::queryOrganizationEx()
{
	TP_DEBUG_TRACE("");
	//获取请求地址
	...
	sprintf_s(strsoapaction,TP_D_SOAPACTION_LEN,"%s:\"%s%s\"",TP_D_HTTP_HEADER_SOAPACTION,TP_D_SOAP_ENVELOPE_ATTR_SMC_VALUE,TP_D_QUERY_ORGANIZATIONEX);
	...
	XMLProcess::SetXml_queryOrganizationEx( request.strReqData);
	...
	retVal.resultCode = XMLProcess::GetXml_queryOrganizationEx(rspBody.c_str(), &retVal.result);
